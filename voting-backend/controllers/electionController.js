...
//   const allCandUserIds = new Set();
//   const nameCache = new Map(); // lower(name) -> resolved user {_id, degree, faculty, program} | null

//   // helper: resolve a missing candidateUserId by exact (case-insensitive) name
//   async function resolveUserByNameIfNeeded(c) {
//     if (c.candidateUserId || !c.name || !c.name.trim()) return null;
//     const key = c.name.trim().toLowerCase();
//     if (!nameCache.has(key)) {
//       const rx = new RegExp(`^${escapeRegExp(c.name.trim())}$`, 'i');
//       const u = await User.findOne({
//         name: rx,
//         isVerified: true,
//         role: { $ne: 'admin' },
//       }).select('_id degree faculty program').lean();
//       nameCache.set(key, u || null);
//     }
//     const u = nameCache.get(key);
//     if (u) {
//       c.candidateUserId = String(u._id);
//       return u;
//     }
//     return null;
//   }
...
//           party,
//         };
//         // fallback: resolve by name when id missing
//         // (safe, exact match, verified only)
//         if (!entry.candidateUserId) {
//           const u = await resolveUserByNameIfNeeded(entry);
//           if (u) allCandUserIds.add(String(u._id));
//         }
//         if (entry.candidateUserId) allCandUserIds.add(String(entry.candidateUserId));
//         pushC(k, entry);
//       }
//     }
...
//       for (const uid of likeSet)  posMap.set(uid, Math.max(posMap.get(uid) || 0, 1 * w));
//       for (const uid of comSet)   posMap.set(uid, Math.max(posMap.get(uid) || 0, 2 * w));
//       for (const uid of disSet)   negMap.set(uid, Math.max(negMap.get(uid) || 0, 1 * w));
//     }
...
//     let totalRaw = scored.reduce((s, r) => s + r.raw, 0);
//     let prior = totalRaw > 0 ? scored.map(r => r.raw / totalRaw) : scored.map(_ => 1 / scored.length);
