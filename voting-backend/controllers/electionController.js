...
//   // Per-reactor best (max) contribution across posts in the window
...
//     // commenters have weight 2*w, likers 1*w (take max if same person did both)
...
//   // combine reactors: score = sum(max(pos) - max(neg)), clipped at >= 0
//   let score = 0;
...
//     // note: later, per-candidate we’ll funnel social parts to program/faculty if they personally have no social
...
//       // Missing social for THIS candidate?
...
//       // candidate-level: if others have social but this candidate has none in that bucket → move to program
//       if (sumPre > 0 && !hasPre) {
...
//       // if no votes in the position, move w.votes mass across buckets
//       if (sumVotes === 0) {
//         eff.votes = 0; // replaced
//         // split across program, faculty, pre(if any), post(if any)
//         let temp = {
...
//         // if this candidate personally lacks social, route social shares 80%→program, 20%→faculty
//         if (!hasPre) {
...
//       // final safety: normalize eff to sum 1
...
//     } else {
//       // completely empty signals → equal prior
...
//     // if multi-seat (“members”), estimate seatProbability
...
//       // seatProbability only meaningful for multi-seat; null otherwise
//       seatProbability: seatProbabilityByIdx ? seatProbabilityByIdx[i] : null,
//       // (optional) transparency for admins/debug
//       breakdown: r.shares,
