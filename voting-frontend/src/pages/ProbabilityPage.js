...
export default function ProbabilityPage() {
  ...
  const [boxes, setBoxes] = useState([]); // [{_id, title, startDate, endDate, probabilities}]

  const fetchOne = useCallback(async (el) => {
    // Try probability; ignore 404 so one bad election won't break page
    let prob = null;
    try {
  ...
  useEffect(() => {
    load();
    const id = setInterval(load, 10000); // auto-refresh 10s
