...
//   const [avail, setAvail] = useState(null); // includes .flat
//   const [destKey, setDestKey] = useState(""); // party:Party A:president | independent:secretary | party:Party A:members
...
//   // when candidate changes, prefill + fetch available slots (flat)
//   useEffect(() => {
//     if (!selectedId) {
//       setSelected(null);
//       setAvail(null);
...
//         // default destination: try to keep current seat if present in flat list; else first available
//         const currPos = normalizePosition(c.position);
//         const currParty = c.partyName === "Independent" ? null : (c.partyName || "");
//         const currKey = currParty
//           ? `party:${currParty}:${currPos}`
//           : `independent:${currPos}`;

//         const exists = (r.data.flat || []).some(f => f.key === currKey);
//         if (exists) setDestKey(currKey);
//         else if ((r.data.flat || []).length) setDestKey(r.data.flat[0].key);
//         else setDestKey("");
//       } catch (e) {
//         console.error("available-slots failed:", e?.response?.data || e.message);
//         alert("Failed to fetch available slots");
//       }
//     })();
//   }, [selectedId, candidates]);

//   // derived replacements: members of source party (excluding self)
...
//     // parse destKey
//     // party:Party A:president | party:Party A:members | independent:secretary | independent:members
//     const [kind, maybeParty, maybeSlot] = destKey.split(":");
//     const destType = kind === "party" ? "party" : "independent";
//     const destPartyName = destType === "party" ? maybeParty : null;
//     const destSlot = destType === "party" ? maybeSlot : maybeParty; // careful: for independent:key format is independent:<slot>
...
//         payload.position  = destSlot; // 'president'|'vicePresident'|'secretary'|'treasurer'|'members'
//       } else {
//         payload.position = "independent";
//         payload.post     = destSlot; // 'president'|'vicePresident'|'secretary'|'treasurer'|'members'
//       }

//       // optional replacement (only if leaving party single-seat)
...
//       // refresh flat
//       const r2 = await api.get("/api/candidates/available-slots", {
//         params: { electionId: selected.electionId, candidateId: selected._id }
//       });
//       setAvail(r2.data);
...
//   if (loading) return <div className="update-candidate-container"><h1>Update Candidate</h1><p>Loading…</p></div>;

//   return (
//     <div className="update-candidate-container">
//       <div className="header">
//         <h1>Update Candidate</h1>
//         <p>Select a candidate, then choose from **all empty seats** (including Members) across the election.</p>
//       </div>
...
//           {/* Destination picker from flat empties */}
//           <div className="row">
//             <div className="col">
//               <label className="label">Move To (empty seats only)</label>
//               <select className="input" value={destKey} onChange={e => setDestKey(e.target.value)} disabled={!avail || !(avail.flat || []).length}>
//                 <option value="">-- Select destination --</option>

//                 {/* Group by party & independent */}
//                 {(() => {
//                   if (!avail?.flat) return null;

//                   const partyGroups = {};
//                   const independent = [];
//                   for (const f of avail.flat) {
//                     if (f.type === "party") {
...
//           {/* Replacement picker (only when leaving a party single-seat) */}
//           {sourceIsPartySingleSeat && (
//             <div className="row">
//               <div className="col">
//                 <label className="label">
//                   Replacement for vacated seat (optional) — members of {selected.partyName}
//                 </label>
...
//                   ))}
//                 </select>
//                 <p className="hint">If selected, this member will be promoted to the seat you’re vacating.</p>
//               </div>
//             </div>
//           )}
  ...
  const handleSubmitNew = async (e) => {
    ...
    try {
      const payload = {
        ...
        // name is optional — backend will pull from verified user
      };
